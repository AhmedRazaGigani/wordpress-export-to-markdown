...
function getPostPath(post, config) {
    ...
    if (config.postFolders) {
        ...
        if (['tribe_venue', 'tribe_organizer', 'tribe_event_series'].includes(post.meta.type)) {
            filename = '_index.md';
        }
        pathSegments.push(slugFragment, filename);
    } else {
        let filename = slugFragment + '.md';
        if (['tribe_venue', 'tribe_organizer', 'tribe_event_series'].includes(post.meta.type)) {
            filename = slugFragment + '/_index.md';
        }
